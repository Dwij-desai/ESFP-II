...
public:
  ...
  // Getter function to access numStudents (Option 1)
...
int main() {
  ...
  cout << "[3] Display all the Databases" << endl;
  ...
  while (true) {
    switch (choice) 
    {
      ...
      case 3:
        ...
        for (int i = 0; i < numDatabases; ++i) {
          cout << "\n** Database " << (i + 1) << " **" << endl;
          // **Fix:** We need to check if the database is created before accessing its methods.
          if (i < numDatabases) { // Check if database exists (avoid potential out-of-bounds access)
            // Option 1: Using getter function
            int studentCount = obj[i].getNumStudents();
            if (studentCount > 0) {
          ...
          // Option 2: Modifying printStudents (less reusable)
          // obj[i].printStudents(); // Would display student information if implemented
        }
